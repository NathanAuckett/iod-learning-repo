...
let studentSirNames = new Map();
students.map(s => {
    let {name: sirname} = s;
    sirname = sirname.split(" ")[1].toUpperCase();

    studentSirNames.set(sirname, studentSirNames.get(sirname) === undefined ? 1 : studentSirNames.get(sirname) + 1);
});
console.log("Sirname counts: " + [...studentSirNames.entries()]);

students.sort((a, b) => {
    let {name: name1} = a;
    name1 = name1.split(" ")[1].toUpperCase();
    let {name: name2} = b;
    name2 = name2.split(" ")[1].toUpperCase();

    return (name1 < name2) ? -1 : (name1 > name2) ? 1 : 0;
});
// console.log("Students sorted by sirname");
...
// 0 = score, 1 = studentObjectReference
...
students.forEach(student => {
    ...
    scores.forEach(s => {
        let t = s.type;
        let n = s.score;
        //check what type is and then check if higher than highest.
    });
});
